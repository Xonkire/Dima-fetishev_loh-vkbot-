...
vk_session = vk_api.VkApi(token="[eqc8352b0deeaabec15b61d95e5c493baa05e5957041ed7738481b8dc1eb38c6b3aa9f7a382aacc93164472")
    ...
    for event in longpoll.listen():
        if is_message(event):
            if is_play_game(event):
                ...
                # TODO исправить, если это говно не будет работать
                if user1 != event.user_id:
                    user1, user2 = user2, user1
                if not igra.is_unused_city(city):
                    send_message(user1, 'Ты проиграл, потому-что назвла город, который уже был.')
                    send_message(user2, 'Вы победили!')
                    kill_game(igra)
                # TODO проверка на гниду(АФК)
